    ...
    >
      <div
        class={cn(
          ...
          ctx.isMobile() ? 'min-h-9 flex items-center' : 'w-14'
        )}
      >
        {props.label}
      </div>
...
export function ComposeRecipients(props: {
  ...
  const recipientSelector = (
    ...
    <RecipientSelector
      ...
      class={cn(
        ...
        ctx.isMobile() && '[&_input]:min-w-16! [&_input]:min-h-9!'
      )}
      noPadding
  ...
  const fieldLabel = (text: string) => (ctx.isMobile() ? `${text}:` : text);
